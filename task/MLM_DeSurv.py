...
def main():
	...
	print(f"Outcome label2idx mapping: {label2idx}")
	...
	for epoch in range(N_EPOCHS):
		...
		for step, batch in enumerate(trainload):
			...
			x_ = pooled_output[argsort_t,:].to(desurv_model.odenet.device).squeeze(1)
			t_ = target_time[argsort_t].to(desurv_model.odenet.device).squeeze(1).squeeze(1)
			k_ = target_label[argsort_t].to(desurv_model.odenet.device).squeeze(1).squeeze(1)
